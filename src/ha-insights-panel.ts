...
export class HaInsightsPanel extends LitElement {
  ...
  @state() private _chatOpen = false;
  @state() private _chatPrompt = "";
  @state() private _chatBusy = false;
  @state() private _chatError = "";
  @state() private _chatResult: {
  ...
  private async _submitChat(): Promise<void> {
    if (!this.hass) return;
   ...
   *  endpoint needed. */
  private async _applyChat(): Promise<void> {
    if (!this.hass || !this._chatResult) return;
    try {
      await this.hass.connection.sendMessagePromise({
      ...
      this._showToast(
        "✅ Automation created. Find it in Settings → Automations.",
      );
      this._closeChat();
    } catch (err) {
      const message = (err as { message?: string }).message ?? String(err);
  ...
  private _renderChatModal(): TemplateResult | "" {
  ...
  private _renderChatInput(): TemplateResult {
    return html`
      ...
      <textarea
        ...
        @input=${(e: Event) =>
          (this._chatPrompt = (e.target as HTMLTextAreaElement).value)}
      ></textarea>
      <div class="chat-counter">${this._chatPrompt.length} / 2000</div>
      ${this._chatError
        ? html`<div class="chat-error">${this._chatError}</div>`
        : ""}
      <div class="diagnostics-actions">
        ...
        >
          ${this._chatBusy
            ? "Generating…"
            : html`<ha-icon icon="mdi:send"></ha-icon> Generate`}
        </button>
      </div>
    `;
  }

  private _renderChatResult(): TemplateResult {
  ...
  private _formatYamlPayload(payload: Record<string, unknown>): string {
    const lines: string[] = [];
    const writeValue = (value: unknown, indent: string): void => {
