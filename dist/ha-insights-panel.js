...
let HaInsightsCardEditor = class HaInsightsCardEditor extends i {
    constructor() {
        super(...arguments);
...
HaInsightsCardEditor = __decorate([
    t("ha-insights-card-editor")
], HaInsightsCardEditor);

var HaInsightsCard_1;
const CARD_PROTOCOL_VERSION = 1;
const DEFAULT_MAX_ROWS = 8;
let HaInsightsCard = class HaInsightsCard extends i {
    constructor() {
        super(...arguments);
  ...
  `; }
    ...
    _updateAutoMaxRows(hostHeight) {
        const available = Math.max(0, hostHeight - HaInsightsCard_1.HEADER_HEIGHT_PX);
        const rows = Math.max(1, Math.floor(available / HaInsightsCard_1.ROW_HEIGHT_PX));
    ...
    async _showBackfillToast() {
        if (!this.hass)
            return;
        try {
            const status = await this.hass.connection.sendMessagePromise({ type: "home_insights/backfill_status" });
            if (status.running) {
                this._toast = "Backfilling history…";
                return;
            }
            const last = status.last;
            if (!last || last.events_added === 0)
                return;
            ...
            if (Date.now() - completed > 5 * 60 * 1000)
                return;
...
HaInsightsCard = HaInsightsCard_1 = __decorate([
    t("ha-insights-card")
], HaInsightsCard);
...
var HaInsightsPanel_1;
let HaInsightsPanel = class HaInsightsPanel extends i {
    constructor() {
        super(...arguments);
        this.narrow = false;
        this._search = "";
  ...
  `; }
    ...
    _loadFilters() {
        try {
            const raw = window.localStorage.getItem(HaInsightsPanel_1._STORAGE_KEY);
            if (!raw)
                return;
    ...
    _saveFilters() {
        try {
            window.localStorage.setItem(HaInsightsPanel_1._STORAGE_KEY, JSON.stringify({
                search: this._search,
...
HaInsightsPanel = HaInsightsPanel_1 = __decorate([
    t("ha-insights-panel")
], HaInsightsPanel);
